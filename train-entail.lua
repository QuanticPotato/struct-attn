        ...
        for i = 1, data:size() do
            ...
            if opt.attn ~= 'none' then
                ...
                for t = source_l, 1, -1 do
                    ...
                    parser_context1[{
                        {}, t,
                        { opt.rnn_size_parser + 1, opt.rnn_size_parser * 2 }
                    }]:copy(out[#out])
                end
                ...
                for t = target_l, 1, -1 do
                    ...
                    parser_context2[{
                        {}, t, {
                            opt.rnn_size_parser + 1,
                            opt.rnn_size_parser * 2
                        }
                    }]:copy(out[#out])
                end
            ...
            if opt.attn ~= 'none' then
                local dl_dinput1, dl_dinput2, dl_dparser1, dl_dparser2 = table.unpack(sent_encoder:backward(pred_input, dl_dp))
                ...
                for t = source_l, 1, -1 do
                    drnn_state[#drnn_state]:add(parser_grads1[{ {}, t, { 1, opt.rnn_size_parser } }])
                ...
                local drnn_state = reset_state(init_parser, batch_l)
                for t = 1, source_l do
                    drnn_state[#drnn_state]:add(parser_grads1[{ {}, t, { opt.rnn_size_parser + 1, 2 * opt.rnn_size_parser } }])
                ...
                local drnn_state = reset_state(init_parser, batch_l)
                for t = target_l, 1, -1 do
                    drnn_state[#drnn_state]:add(parser_grads2[{ {}, t, { 1, opt.rnn_size_parser } }])
                ...
                local drnn_state = reset_state(init_parser, batch_l)
                for t = 1, target_l do
                    drnn_state[#drnn_state]:add(parser_grads2[{ {}, t, { opt.rnn_size_parser + 1, 2 * opt.rnn_size_parser } }])
